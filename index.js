...
function initializeGame() {
  ...
  statusText.innerHTML = arlTag() + `'s turn`;
...
function arlTag() {
  const turncolor = currentPlayer == "●" ? "black" : "white";
  return `<span class='arl ${turncolor}'></span>`;
}
...
function checkWinner() {
    ...
    for (let x = 0; x < 20; x++) {
      ...
      if (options[y * 20 + x] == currentPlayer) {
        ...
        if (y == 0 || (y > 0 && options[(y - 1) * 20 + x] != currentPlayer)) {
          ...
          if (
            y == 15 ||
            (y < 15 && options[(y + 5) * 20 + x] != currentPlayer)
          ) {
            //4칸연속 현플레이어이면 승리조건충족
            roundWon = true;
            for (let i = 0; i < 4; i++) {
        ...
        if (
          x == 0 ||
          y == 0 ||
          (x > 0 && y > 0 && options[(y - 1) * 20 + (x - 1)] != currentPlayer)
        ) {
          ...
          if (
            ...
            (y < 15 &&
              x < 15 &&
              options[(y + 5) * 20 + (x + 5)] != currentPlayer)
          ) {
            //4칸연속 현플레이어이면 승리조건충족
            roundWon = true;
            for (let i = 0; i < 4; i++) {
        ...
        if (
          x == 19 ||
          y == 19 ||
          (x < 19 && y < 19 && options[(y - 1) * 20 + (x + 1)] != currentPlayer)
        ) {
          //y축이동공간이 5칸 || y축이동공간 6칸이상이면서 (6칸) != 현플레이어
          if (
            x == 4 ||
            y == 4 ||
            (y > 4 && x > 4 && options[(y + 5) * 20 + (x - 5)] != currentPlayer)
          ) {
            //4칸연속 현플레이어이면 승리조건충족
            roundWon = true;
            for (let i = 0; i < 4; i++) {
  ...
  if (roundWon) {
    ...
    statusText.innerHTML = arlTag() + `wins !!`;
    running = false;
